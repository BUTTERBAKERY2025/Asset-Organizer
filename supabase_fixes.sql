...
-- تاريخ الإنشاء: 2026-03-03 (محدّث)
...
-- =============================================
-- 2. حذف تكرارات الورديات قبل إنشاء الفهرس الفريد
-- (يحتفظ بأحدث سجل ويحذف الأقدم)
-- =============================================
DELETE FROM branch_shift_profiles a
USING branch_shift_profiles b
WHERE a.branch_id = b.branch_id 
  AND a.shift_code = b.shift_code 
  AND a.id < b.id;

-- =============================================
-- 3. إضافة حماية ضد تكرار الورديات لنفس الفرع
-- (الآن سينجح بعد حذف التكرارات)
-- =============================================
CREATE UNIQUE INDEX IF NOT EXISTS idx_unique_shift_profile_per_branch 
ON branch_shift_profiles (branch_id, shift_code);

-- =============================================
-- 4. حذف الفهارس المكررة (توفير مساحة)
-- =============================================
DROP INDEX IF EXISTS idx_attendance_records_branch;
DROP INDEX IF EXISTS idx_attendance_records_date;

-- =============================================
-- 5. تحديث أيام الإجازة بدون نوع وردية
-- =============================================
...
-- =============================================
-- 6. إنشاء إعدادات الورديات الافتراضية لجميع الفروع التي لا تملكها
...
-- =============================================
-- 7. فحص: التأكد من أن جميع الفروع لديها إعدادات ورديات
...
-- =============================================
-- 8. فحص: التأكد من صحة أوقات الورديات لكل فرع
-- (هذا استعلام فحص فقط - لا يعدل شيء)
