...
- **Timesheet Reports Dashboard (المرحلة 1 من إعادة بناء `/timesheet`)**: New "لوحة التحكم" tab is the default landing page. Layout: (1) shared filter bar at top (branch + month) used by dashboard + branch-PDF + bulk generation; (2) 6 KPI cards (إجمالي الموظفين / مكتمل وموقّع / بانتظار المدير / بانتظار الموظف / مسوّدة / غير منشأ) computed client-side from `/api/timesheet-reports?branchId=X` filtered to the selected month's `startDate`/`endDate`; (3) quick-actions bar with "إنشاء تقارير لكل من لم يُنشأ (N)" button + branch PDF download; (4) per-employee status table sorted by status priority (not_generated → pending → pending_employee_signature → pending_manager_signature → finalized) with inline action buttons (إنشاء for missing, عرض/توقيع for existing). Shows a "select branch" placeholder when `selectedBranch === "all"`. Backend adds `POST /api/timesheet-reports/generate-bulk` (body: `branchId`, `startDate`, `endDate`, `employeeIds[]`) that pre-fetches schedules + attendance once for the whole branch+range to avoid N+1, loops per employee skipping those with existing reports for that period, and returns `{ created[], skipped[], failed[], summary }`. The bulk endpoint reuses the same daily-entry computation logic as the single-employee `/generate` endpoint and resolves both `branch_emp_X` IDs and user UUIDs (auto-mapping linked branch employees so schedules/attendance saved against `branchEmployeeId` are matched). Tabs: dashboard (default) | view (existing, shown after selecting a report) | history (existing, branch filter now controlled by shared bar). No schema change required.
...
- **Manual Salary Deductions & Advances (السُلف والخصومات اليدوية)**: Dedicated `salary_deductions` table (`shared/schema.ts`) with `branchEmployeeId`, `branchId`, `month` (YYYY-MM), `type` (advance/deduction/loan_installment/penalty/other), `amount`, optional `description`, and `createdBy`. CRUD endpoints under `/api/salary-deductions` (GET/POST/PUT/DELETE) gated by `branch_employees` view/edit permission with `canAccessBranch` check. The `/api/employee-reports/bundle` returns a 5th parallel `salaryDeductions` payload alongside employees/attendance/schedules/signedTimesheets. In the salary closing dialog, a new orange "سُلف/خصومات" column shows a popover (`DeductionsPopover`) per employee allowing add/delete of monthly entries; the total is automatically subtracted from net salary (`net = max(0, gross − insurance − absenceDeduction − manualDeductionsTotal)`). The 6-card summary now includes a "سُلف وخصومات" card. Excel export adds `Manual Deductions` and `Deductions Detail` columns; PDF adds a dedicated "سُلف/خصومات" column showing total + per-entry breakdown plus a totals row + summary line. **Schema change**: requires creating `salary_deductions` table + 4 indexes manually in Supabase before production deploy.
- **Monthly Salary Closing**: Three-tier source priority for each employee: (1) **Signed Timesheet (مصدر الحقيقة)** — when a finalized `timesheet_reports` row overlaps the closing month, its `timesheet_report_entries` (status: present/absent/late/day_off + actualHours) become the authoritative source; (2) **Schedule + Attendance** — `employee_schedules` for expected work/off days hybridized with `attendance_records`; (3) **Attendance only** fallback. Each row carries a `dataSource` flag and the closing dialog renders per-employee badges (✓ موقّع emerald / جدول blue / بصمة فقط orange) plus a top summary banner with the % signed and a recommendation to sign timesheets when not 100%. Backend exposes `getFinalizedTimesheetEntriesByBranchAndDateRange` (server/storage.ts) and the `/api/employee-reports/bundle` endpoint returns a 4th parallel `signedTimesheets` payload alongside employees/attendance/schedules. The dialog uses its own dedicated bundle query keyed on `salaryClosingBranch` + `salaryClosingMonth` (separate from the page-level filters), so changing the closing month/branch always refetches signed timesheets, schedules, and attendance. **Absence deduction** is automatically applied: daily rate = gross salary (base + all allowances) ÷ 30 (Saudi Labor Law convention), absence deduction = absent days × daily rate. Net salary = gross − social insurance − absence deduction. **No-work-at-all rule**: if an employee has zero presentDays AND zero scheduledWorkDays AND zero offDays AND zero attendance records for the closing month, they are treated as absent for the full 30 days (absence deduction = gross − insurance, net = 0). The row is highlighted red and shows a "⚠️ غائب الشهر كامل" badge so the closer can review and either approve, link a missing schedule/timesheet, or convert the case to paid/unpaid leave. UI shows 5 summary cards (employee count, gross, absence deduction, insurance, net) and dedicated "قيمة اليوم" / "خصم الغياب" columns; Excel and PDF exports include the same breakdown plus a "مصدر البيانات" column and a per-PDF data-source summary box. Resolution of timesheet → employee uses both `branchEmployeeId` (integer) and the varchar `employeeId` (which may hold a user UUID or `branch_emp_X`), keeping the latest-signed report when multiple overlap.
