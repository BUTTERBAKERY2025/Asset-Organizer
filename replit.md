...
- **Contractor Account Statements**: Unified page at `/contractor-statements` listing all contractors with summary KPIs (contracts count, total value, paid, balance), with detailed statement at `/contractors/:id/statement` aggregating contract payments, paid/pending payment requests, and direct project expenses, with date range filter, Excel export, and print-friendly view.
- **Project Daily Work Logs**: Daily work logs per project at `/construction/daily-logs` (list with project/date filters) and `/construction/daily-logs/new|/:id/edit` (create/edit form with photo uploads via Supabase Storage). Multiple supervisors can log the same day. Photos categorized as before/during/after. Printable A4 report at `/construction/daily-logs/:id/print` with project info, work description, equipment, weather, issues, photos, and signature blocks. Three new permission modules: `project_expenses`, `contractor_statements`, `project_daily_logs`.
