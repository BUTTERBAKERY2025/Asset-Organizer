...
async function notifyAuditTeam(title: string, content: string, ctx: Ctx) {
  // The shared creator persists the bell notification and dispatches Web Push.
  // Do not report a saved requirement as failed if notification delivery fails.
  try {
    await storage.createSystemNotification({
      ...
      buttonText: "فتح بوابة المراجعة",
      buttonAction: "/audit-portal",
      autoGenerated: true,
      autoSource: "audit_portal",
      createdBy: ctx.id,
    });
  } catch (error) {
    console.error("[audit-portal] Failed to notify financial team:", error);
  }
}
...
export function registerAuditPortalRoutes(app: Express) {
  ...
  app.post("/api/audit/periods/:id/requirements", isAuthenticated, requireTeamOrAuditor, async (req: any, res) => {
    try {
      ...
      if (row.source === "auditor") {
        await notifyAuditTeam("طلب جديد من المراجع الخارجي", `أضاف ${ctx.name} متطلباً جديداً: ${row.title}`, ctx);
      }
      res.json(row);
    } catch (e: any) { res.status(500).json({ error: e.message }); }
  });
  ...
  app.patch("/api/audit/requirements/:id", isAuthenticated, requireTeamOrAuditor, async (req: any, res) => {
    try {
      ...
      if (patch.status === "rejected" && r.status !== "rejected") {
        await notifyAuditTeam("إرجاع بند من المراجع الخارجي", `أرجع ${ctx.name} البند للتعديل: ${r.title}`, ctx);
      }
      res.json(row);
    } catch (e: any) { res.status(500).json({ error: e.message }); }
  });
