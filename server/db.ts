...
export async function runStartupMigrations() {
  try {
    ...
    const migrations = [
      ...
      // دورة حياة السلف الكاملة: مراجعة + توقيع + اعتماد نهائي + صرف + سلف سابقة
    ...
    // The trigger starts recording only NEW, explicitly addressed personal notifications.
    // No historical rows are scanned/backfilled. Keep this in sync with migration 038.
    try {
      await pool.query(`
      ...
      `);
    } catch (personalPushErr: any) {
      console.error("[PUSH] Personal notification outbox migration error:", personalPushErr?.message);
    }
