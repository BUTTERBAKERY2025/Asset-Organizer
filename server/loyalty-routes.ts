...
export function registerLoyaltyRoutes(app: Express) {
  ...
  // Public registration — upsert customer by phone, issue (or reuse) member code
  ...
  app.post("/api/public/loyalty/:slug/register", async (req, res) => {
    try {
      ...
      const result = await db.transaction(async (tx) => {
        ...
        // Race-safe insert: if a concurrent request already created the
        // membership (unique campaignId+customerId), do nothing and re-select.
        ...
        // Lost the race — fetch the membership the other request created
