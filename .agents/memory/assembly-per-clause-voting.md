...
- **Clause freeze**: admin edit/delete of a clause must be blocked once *any* vote exists on the **parent resolution** (count by `resolutionId`), not per-clause. Otherwise the ballot could change mid-vote.
