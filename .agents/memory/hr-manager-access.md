...
- **Granularity caveat:** most `/api/hr/*` routes call `requirePermission("hr_*")` with NO action arg → `action===undefined` → for an included module the grant is effectively module-level (e.g. specialist CAN hit leave review/approve). True per-action limits only work where the route passes an explicit action (e.g. `branch_employees` delete → blocked since delete is not in the map; create/edit ARE allowed). To restrict further, the HR route itself must pass an action string.
