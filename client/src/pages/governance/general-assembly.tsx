      ...
      <Dialog open={showNewMeeting} onOpenChange={setShowNewMeeting}>
        <DialogContent className="max-w-2xl">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="col-span-2">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="flex items-center gap-3 p-3 border rounded-lg bg-green-50 border-green-200">
                  <Checkbox
                    id="sendWhatsApp"
                    checked={newMeeting.sendWhatsApp}
                    onCheckedChange={(checked) => setNewMeeting({ ...newMeeting, sendWhatsApp: !!checked })}
                  />
                  <label htmlFor="sendWhatsApp" className="flex items-center gap-2 cursor-pointer">
                    ...
                    <span className="font-medium text-green-700">واتساب</span>
                  </label>
                </div>
                <div className="flex items-center gap-3 p-3 border rounded-lg bg-blue-50 border-blue-200">
                  <Checkbox
                    id="sendEmail"
                    checked={newMeeting.sendEmail}
                    onCheckedChange={(checked) => setNewMeeting({ ...newMeeting, sendEmail: !!checked })}
                  />
                ...
                <div className="flex items-center gap-3 p-3 border rounded-lg bg-purple-50 border-purple-200">
                  <Checkbox
                    id="sendSMS"
                    checked={newMeeting.sendSMS}
                    onCheckedChange={(checked) => setNewMeeting({ ...newMeeting, sendSMS: !!checked })}
                  />
                  <label htmlFor="sendSMS" className="flex items-center gap-2 cursor-pointer">
                    <Phone className="h-5 w-5 text-purple-600" />
                    <span className="font-medium text-purple-700">رسالة نصية SMS</span>
                  </label>
                </div>
              </div>
            </div>
            ...
            <div className="col-span-2">
              <Label>نص الدعوة (اختياري)</Label>
              <Textarea
                value={newMeeting.invitationMessage}
                onChange={(e) => setNewMeeting({ ...newMeeting, invitationMessage: e.target.value })}
                placeholder="نص مخصص للدعوة سيتم إرساله مع تفاصيل الاجتماع..."
                rows={3}
              />
              <p className="text-xs text-gray-500 mt-1">
                سيتم إرسال الدعوة تلقائياً إلى {shareholders.filter(s => s.votingRights).length} مساهم لهم حق التصويت
              </p>
            </div>
          </div>
          <DialogFooter className="flex justify-between items-center">
            <div className="text-sm text-gray-500">
              {(newMeeting.sendWhatsApp || newMeeting.sendEmail || newMeeting.sendSMS) && (
                <span className="flex items-center gap-1">
                  <Bell className="h-4 w-4" />
                  سيتم إرسال الدعوات عند الإنشاء
                </span>
              )}
            </div>
            <div className="flex gap-2">
              ...
              <Button 
                ...
                className="gap-2"
              >
                {createMeetingMutation.isPending ? 'جاري الإنشاء...' : (
                  <>
                    <Plus className="h-4 w-4" />
