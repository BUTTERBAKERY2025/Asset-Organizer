    ...
    >
      <CardHeader className="p-4 pb-3">
        <div className="flex items-center gap-3">
          <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${color} transition-transform group-hover:scale-110 shrink-0`}>
        ...
        <CardDescription className="text-sm leading-snug line-clamp-2 mt-2">{description}</CardDescription>
      </CardHeader>
          ...
          <div className="flex flex-wrap gap-1.5">
            {items.slice(0, 3).map((item, index) => (
              <Link key={index} href={item.href} onClick={(e) => e.stopPropagation()}>
                <Badge 
                  variant="outline" 
                  className="flex items-center gap-1 hover:bg-secondary cursor-pointer transition-colors text-xs px-2 py-1"
                >
