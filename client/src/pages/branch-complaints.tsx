        ...
        <header className="pt-5 sm:pt-8">
          <div className="flex flex-col gap-4 border-b border-border pb-5 sm:flex-row sm:items-end sm:justify-between">
            <div className="flex items-start gap-3">
              <span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-primary text-primary-foreground"><MessageSquareWarning className="h-6 w-6" /></span>
              <div><p className="text-xs font-bold tracking-[.15em] text-primary">BUTTER BAKERY · BRANCH CARE</p><h1 className="mt-1 text-2xl font-black text-foreground sm:text-3xl">شكاوى الفروع</h1><p className="mt-1 text-sm text-muted-foreground">متابعة البلاغ من الاستلام حتى توثيق الحل والإغلاق.</p></div>
            </div>
            <div className="flex gap-2">
              ...
              {createAllowed && <Button className="bg-primary hover:bg-primary/90" onClick={() => { setEditing(null); setFormOpen(true); }} data-testid="button-create-complaint"><Plus className="ml-2 h-4 w-4" />بلاغ جديد</Button>}
            </div>
          </div>
          <div className="mt-4 grid gap-3 rounded-2xl border border-border bg-card p-3 sm:grid-cols-3">
...
function ComplaintCard({ item, onOpen }: { item: Complaint; onOpen: () => void }) {
  return <button type="button" onClick={onOpen} className="w-full rounded-2xl border border-border bg-card p-4 text-right shadow-sm transition hover:border-primary/40 hover:shadow-md" data-testid={`complaint-${item.id}`}>
    <div className="flex items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><StatusBadge status={item.status} /><PriorityBadge priority={item.priority} /><span className="text-xs text-muted-foreground">#{item.id.toLocaleString("en-US")}</span></div><h2 className="mt-2 font-black text-foreground">{item.subject}</h2></div><ArrowRight className="mt-1 h-5 w-5 rotate-180 text-primary" /></div>
    <div className="mt-3 flex flex-wrap gap-x-5 gap-y-1 text-xs text-muted-foreground"><span>{categoryLabels[item.category]}</span><span><UserRound className="ml-1 inline h-3.5 w-3.5" />{item.ownerName ?? "غير مسند"}</span><span><Clock3 className="ml-1 inline h-3.5 w-3.5" />الاستجابة: {formatDate(item.responseDue)}</span></div>
  </button>;
}
...
function ComplaintForm({ open, onOpenChange, branchId, complaint, onConflict, onSaved }: { open: boolean; onOpenChange: (open: boolean) => void; branchId: string | null; complaint: Detail | null; onConflict: () => void; onSaved: (value: Complaint) => void }) {
  ...
  </div><DialogFooter className="border-t p-4"><Button type="button" variant="outline" onClick={() => onOpenChange(false)} disabled={save.isPending}>إلغاء</Button><Button type="submit" className="bg-primary hover:bg-primary/90" disabled={save.isPending}>{save.isPending && <Loader2 className="ml-2 h-4 w-4 animate-spin" />}حفظ</Button></DialogFooter></form></DialogContent></Dialog>;
}
...
function DetailDialog({ open, onOpenChange, detail, loading, error, canEdit, canApprove, onRetry, onEdit, onTransition, onChanged }: { open: boolean; onOpenChange: (open: boolean) => void; detail?: Detail; loading: boolean; error: string | null; canEdit: boolean; canApprove: boolean; onRetry: () => void; onEdit: () => void; onTransition: (action: "start" | "resolve" | "close" | "reopen", label: string) => void; onChanged: () => void }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent dir="rtl" className="flex max-w-3xl flex-col overflow-hidden p-0" style={style}><DialogHeader className="border-b p-5 text-right"><DialogTitle>{detail ? `البلاغ #${detail.id.toLocaleString("en-US")} · ${detail.subject}` : "تفاصيل البلاغ"}</DialogTitle></DialogHeader><div className="min-h-0 flex-1 overflow-y-auto p-5">
    ...
    {detail && !loading && <>
      ...
      <section className="mt-5 rounded-2xl bg-muted p-4"><h3 className="font-black text-foreground">نظرة عامة</h3><p className="mt-3 whitespace-pre-wrap text-sm leading-7 text-foreground">{detail.description}</p><dl className="mt-4 grid gap-3 text-sm sm:grid-cols-2"><div><dt className="text-muted-foreground">التصنيف</dt><dd className="font-bold">{categoryLabels[detail.category]}</dd></div><div><dt className="text-muted-foreground">المسؤول</dt><dd className="font-bold">{detail.ownerName ?? "غير مسند"}</dd></div><div><dt className="text-muted-foreground">موعد الاستجابة</dt><dd className="font-bold">{formatDate(detail.responseDue)}</dd></div><div><dt className="text-muted-foreground">تاريخ الإنشاء</dt><dd className="font-bold">{formatDate(detail.createdAt)}</dd></div></dl></section>
      <section className="mt-5"><h3 className="font-black text-foreground">الإجراءات</h3><div className="mt-2 flex flex-wrap gap-2">{canEdit && detail.status === "open" && <Button onClick={() => onTransition("start", "بدء المعالجة")}><Clock3 className="ml-2 h-4 w-4" />بدء المعالجة</Button>}{canEdit && detail.status === "in_progress" && <Button onClick={() => onTransition("resolve", "تسجيل الحل")}><CheckCircle2 className="ml-2 h-4 w-4" />تسجيل الحل</Button>}{canApprove && detail.status === "resolved" && <Button onClick={() => onTransition("close", "إغلاق البلاغ")}><CheckCircle2 className="ml-2 h-4 w-4" />إغلاق البلاغ</Button>}{canApprove && (detail.status === "closed" || detail.status === "resolved") && <Button variant="outline" onClick={() => onTransition("reopen", "إعادة فتح البلاغ")}><RotateCcw className="ml-2 h-4 w-4" />إعادة فتح</Button>}</div></section>
      <Attachments complaint={detail} canEdit={canEdit} onChanged={onChanged} />
      <section className="mt-6"><h3 className="flex items-center gap-2 font-black text-foreground"><History className="h-5 w-5" />سجل البلاغ</h3><div className="mt-3 space-y-3">{detail.history.length === 0 ? <p className="text-sm text-muted-foreground">لا توجد أحداث مسجلة.</p> : detail.history.map((event) => <div key={event.id} className="border-r-2 border-primary/20 pr-3"><div className="flex flex-wrap items-center gap-2"><b className="text-sm">{eventLabels[event.eventType] ?? event.eventType}</b><span className="text-xs text-muted-foreground">{formatDate(event.createdAt)}</span></div><p className="mt-1 text-xs text-muted-foreground">{event.actorName ?? "مستخدم النظام"}{event.fromStatus && event.toStatus ? ` · ${statusLabels[event.fromStatus]} ← ${statusLabels[event.toStatus]}` : ""}</p>{event.reason && <p className="mt-1 rounded-lg bg-muted p-2 text-sm">{event.reason}</p>}</div>)}</div></section>
    </>}
...
function Attachments({ complaint, canEdit, onChanged }: { complaint: Detail; canEdit: boolean; onChanged: () => void }) {
  ...
  return <section className="mt-6"><div className="flex items-center justify-between gap-2"><h3 className="flex items-center gap-2 font-black text-foreground"><Paperclip className="h-5 w-5" />المرفقات</h3>{canEdit && <><input ref={inputRef} type="file" className="hidden" onChange={(e) => upload(e.target.files?.[0])} /><Button size="sm" variant="outline" disabled={progress !== null} onClick={() => inputRef.current?.click()}><Upload className="ml-2 h-4 w-4" />إضافة ملف</Button></>}</div>
    {progress !== null && <div className="mt-3"><div className="mb-1 flex justify-between text-xs"><span>جار الرفع</span><span>{progress.toLocaleString("en-US")}%</span></div><div className="h-2 overflow-hidden rounded-full bg-muted"><div className="h-full bg-primary" style={{ width: `${progress}%` }} /></div></div>}
    {error && <p className="mt-2 text-sm font-semibold text-red-700" role="alert">{error}</p>}
    <div className="mt-3 space-y-2">{complaint.attachments.length === 0 ? <p className="text-sm text-muted-foreground">لا توجد مرفقات.</p> : complaint.attachments.map((file) => <div key={file.id} className="flex items-center gap-3 rounded-xl border bg-white p-3"><Paperclip className="h-5 w-5 text-primary" /><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold">{file.originalName}</p><p className="text-xs text-muted-foreground">{file.sizeBytes ? `${(file.sizeBytes / 1024).toLocaleString("en-US", { maximumFractionDigits: 1 })} KB · ` : ""}{formatDate(file.createdAt)}</p></div><Button asChild size="icon" variant="ghost"><a href={`/api/branch-complaints/${complaint.id}/attachments/${file.id}`} aria-label={`تنزيل ${file.originalName}`}><Download className="h-4 w-4" /></a></Button>{canEdit && <Button size="icon" variant="ghost" className="text-red-700" disabled={deleting !== null} onClick={() => remove(file)} aria-label={`حذف ${file.originalName}`}>{deleting === file.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />}</Button>}</div>)}</div>
  </section>;
}
...
function TransitionDialog({ open, value, complaint, onOpenChange, onConflict, onSaved }: { open: boolean; value: { action: "start" | "resolve" | "close" | "reopen"; label: string } | null; complaint?: Detail; onOpenChange: (open: boolean) => void; onConflict: () => void; onSaved: () => void }) {
  ...
  return <Dialog open={open} onOpenChange={(next) => { if (!mutation.isPending) onOpenChange(next); }}><DialogContent dir="rtl" className="max-w-md" style={style}><DialogHeader className="text-right"><DialogTitle>{value?.label}</DialogTitle></DialogHeader>{error && <Alert variant="destructive"><AlertCircle className="h-4 w-4" /><AlertDescription>{error}</AlertDescription></Alert>}<div><Label htmlFor="transition-reason">{needsReason ? "السبب / تفاصيل الحل" : "ملاحظة (اختيارية)"}</Label><Textarea id="transition-reason" value={reason} onChange={(e) => setReason(e.target.value)} rows={4} className="mt-1" /></div><DialogFooter><Button variant="outline" onClick={() => onOpenChange(false)} disabled={mutation.isPending}>إلغاء</Button><Button className="bg-primary hover:bg-primary/90" disabled={mutation.isPending || (needsReason && !reason.trim())} onClick={() => mutation.mutate()}>{mutation.isPending && <Loader2 className="ml-2 h-4 w-4 animate-spin" />}{value?.label}</Button></DialogFooter></DialogContent></Dialog>;
}

function Empty({ icon: Icon, title, action }: { icon: typeof AlertCircle; title: string; action?: () => void }) {
  return <section className="mt-8 rounded-3xl border border-dashed border-border bg-card px-5 py-12 text-center"><Icon className="mx-auto h-9 w-9 text-primary" /><h2 className="mt-3 font-black text-foreground">{title}</h2>{action && <Button variant="outline" className="mt-4" onClick={action}><RefreshCw className="ml-2 h-4 w-4" />إعادة المحاولة</Button>}</section>;
}

function ListSkeleton() {
  return <div className="mt-6 space-y-3" aria-label="جار تحميل الشكاوى">{Array.from({ length: 4 }).map((_, index) => <div key={index} className="h-28 animate-pulse rounded-2xl bg-muted" />)}</div>;
}