...
interface MediaAsset {
  ...
  publishDate: string | null; designer: string | null; uploadedBy: number | null;
  createdAt: string;
}
  ...
  return (
    <Layout>
      <div className="space-y-6 p-4">
        <PageHeader
          title="فريق التصوير والميديا"
          subtitle="مكتبة بصرية موحّدة لكل ما يخص الصورة الذهنية للعلامة التجارية — الهوية، بنك الصور، منتجات الفريق، القوالب، والأرشيف"
          icon={Camera}
        />

        <Tabs value={tab} onValueChange={setTab} dir="rtl">
          <TabsList className="grid w-full grid-cols-2 md:grid-cols-5 h-auto">
            <TabsTrigger value="identity" data-testid="tab-identity"><Palette className="w-4 h-4 ml-1" /> الهوية البصرية</TabsTrigger>
            <TabsTrigger value="photos" data-testid="tab-photos"><ImageIcon className="w-4 h-4 ml-1" /> بنك الصور</TabsTrigger>
            <TabsTrigger value="products" data-testid="tab-products"><Sparkles className="w-4 h-4 ml-1" /> منتجات الفريق</TabsTrigger>
            <TabsTrigger value="templates" data-testid="tab-templates"><FileImage className="w-4 h-4 ml-1" /> القوالب</TabsTrigger>
            <TabsTrigger value="archive" data-testid="tab-archive"><FolderArchive className="w-4 h-4 ml-1" /> الأرشيف الخام</TabsTrigger>
          </TabsList>

          <TabsContent value="identity" className="mt-6"><IdentityTab /></TabsContent>
          <TabsContent value="photos" className="mt-6"><AssetGalleryTab category="photos" allowVideo={false} /></TabsContent>
          <TabsContent value="products" className="mt-6"><AssetGalleryTab category="products" allowVideo={true} showPlatform /></TabsContent>
          <TabsContent value="templates" className="mt-6"><AssetGalleryTab category="templates" allowVideo={false} acceptDesign /></TabsContent>
          <TabsContent value="archive" className="mt-6"><AssetGalleryTab category="archive" allowVideo={true} /></TabsContent>
        </Tabs>
      </div>
    </Layout>
  );
...
function IdentityTab() {
  ...
  const { data: logoAssets = [] } = useQuery<MediaAsset[]>({ queryKey: ["/api/media/assets", { category: "identity" }], queryFn: async () => {
    const r = await fetch("/api/media/assets?category=identity"); return r.json();
  }});
    ...
    <div className="space-y-8">
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle className="flex items-center gap-2"><ImageIcon className="w-5 h-5 text-amber-600" /> اللوجو الرسمي</CardTitle>
          <Badge variant="outline">ملفات اللوجو في قسم "الأرشيف الخام" أو "القوالب"</Badge>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <LogoPreview label="على الكريمي" bg="#FFF8EE" />
            <LogoPreview label="على الأبيض" bg="#FFFFFF" />
            <LogoPreview label="على الذهبي" bg="#D4A574" />
            <LogoPreview label="على الأسود" bg="#1A1A1A" />
          </div>
          {logoAssets.length > 0 && (
            <div className="mt-4 text-sm text-slate-600">
              يوجد <strong>{logoAssets.length}</strong> ملف لوجو/هوية مرفوع في بنك التخزين.
            </div>
          )}
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle className="flex items-center gap-2"><Palette className="w-5 h-5 text-amber-600" /> لوحة الألوان الرسمية</CardTitle>
          <Button size="sm" onClick={() => { setEditColor(null); setColorOpen(true); }} data-testid="btn-add-color">
            <Plus className="w-4 h-4 ml-1" /> إضافة لون
          </Button>
        </CardHeader>
        <CardContent>
          {colors.length === 0 ? (
            <div className="text-center py-8 text-slate-500">لا توجد ألوان مسجّلة بعد.</div>
          ) : (
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
              {colors.map(c => (
                <div key={c.id} className="border rounded-lg overflow-hidden bg-white" data-testid={`color-${c.id}`}>
                  <div className="h-24 cursor-pointer relative group" style={{ backgroundColor: c.hex }} onClick={() => copyHex(c.hex)}>
                  ...
                  <div className="p-3">
                    ...
                    <div className="flex items-center justify-between mt-1">
                      <code className="text-xs text-slate-600 cursor-pointer" onClick={() => copyHex(c.hex)}>{c.hex.toUpperCase()}</code>
                      <div className="flex gap-1">
                        <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => { setEditColor(c); setColorOpen(true); }}>
                          <Pencil className="w-3.5 h-3.5" />
                        </Button>
                        <Button size="icon" variant="ghost" className="h-7 w-7 text-red-600" onClick={() => { if (confirm("حذف اللون؟")) delColor.mutate(c.id); }}>
                          <Trash2 className="w-3.5 h-3.5" />
                        </Button>
                      </div>
                    </div>
                    {c.usage && <div className="text-xs text-slate-500 mt-1">{c.usage}</div>}
                  </div>
                </div>
              ))}
            </div>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle className="flex items-center gap-2"><Type className="w-5 h-5 text-amber-600" /> الخطوط المعتمدة</CardTitle>
          <Button size="sm" onClick={() => { setEditFont(null); setFontOpen(true); }} data-testid="btn-add-font">
            <Plus className="w-4 h-4 ml-1" /> إضافة خط
          </Button>
        </CardHeader>
        <CardContent>
          {fonts.length === 0 ? (
            <div className="text-center py-8 text-slate-500">لا توجد خطوط مسجّلة بعد.</div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {fonts.map(f => (
                <div key={f.id} className="border rounded-lg p-4 bg-white" data-testid={`font-${f.id}`}>
                  <div className="flex items-start justify-between mb-2">
                    <div>
                      <div className="font-bold text-lg">{f.name}</div>
                      <div className="text-xs text-slate-500">{f.language === "ar" ? "عربي" : f.language === "en" ? "إنجليزي" : "متعدد"}</div>
                    </div>
                    <div className="flex gap-1">
                  ...
                  <div className="text-2xl my-3" style={{ fontFamily: f.family }}>أبجد هوز حطي كلمن — Aa Bb Cc 123</div>
      ...
      {/* حوارات */}
      <ColorDialog open={colorOpen} onClose={() => { setColorOpen(false); setEditColor(null); }} initial={editColor} onSave={(v) => saveColor.mutate(v)} saving={saveColor.isPending} />
      <FontDialog open={fontOpen} onClose={() => { setFontOpen(false); setEditFont(null); }} initial={editFont} onSave={(v) => saveFont.mutate(v)} saving={saveFont.isPending} />
    </div>
  );
}
...
function LogoPreview({ label, bg }: { label: string; bg: string }) {
  return (
    <div className="border rounded-lg overflow-hidden">
      ...
      <div className="text-xs text-center py-2 bg-white">{label}</div>
    </div>
  );
}
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent dir="rtl">
        ...
        <div className="space-y-3">
          ...
          <div><Label>الاستخدام (Primary, CTA, Background...)</Label><Input value={form.usage || ""} onChange={e => setForm({ ...form, usage: e.target.value })} /></div>
        ...
        <DialogFooter>
          ...
          <Button onClick={() => onSave(form)} disabled={saving || !form.name || !form.hex} data-testid="btn-save-color">
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent dir="rtl">
        ...
        <DialogFooter>
          <Button variant="outline" onClick={onClose}>إلغاء</Button>
          <Button onClick={() => onSave(form)} disabled={saving || !form.name || !form.family} data-testid="btn-save-font">
            {saving && <Loader2 className="w-4 h-4 animate-spin ml-2" />} حفظ
          </Button>
        </DialogFooter>
      </DialogContent>
...
/* ============================== معرض الأصول (مشترك بين 4 تبويبات) ============================== */
function AssetGalleryTab({ category, allowVideo, acceptDesign, showPlatform }: { category: string; allowVideo: boolean; acceptDesign?: boolean; showPlatform?: boolean }) {
  const { toast } = useToast();
  const qc = useQueryClient();
  ...
  const queryKey = ["/api/media/assets", category, q, platform];
  ...
  return (
    <div className="space-y-4">
        ...
        <CardContent className="p-4">
          {isLoading ? (
            ...
            <div className="text-center py-12 text-slate-500">
              ...
              <p>لا توجد ملفات بعد في "{CATEGORY_LABELS[category]}"</p>
              <p className="text-xs mt-1">اضغط "رفع جديد" لإضافة أول ملف</p>
            </div>
          ) : (
      ...
      <UploadDialog
        ...
        onUploaded={() => { qc.invalidateQueries({ queryKey: ["/api/media/assets"] }); setUploadOpen(false); }}
      />
...
function AssetCard({ asset, onView, onEdit, onDelete }: { asset: MediaAsset; onView: () => void; onEdit: () => void; onDelete: () => void }) {
  ...
  return (
    <div className="group border rounded-lg overflow-hidden bg-white hover:shadow-lg transition" data-testid={`asset-${asset.id}`}>
      <div className="aspect-square bg-slate-100 relative cursor-pointer" onClick={onView}>
        {isImage ? (
          ...
          <div className="w-full h-full flex items-center justify-center bg-slate-800 text-white relative">
            ...
            <Badge className="absolute bottom-1 right-1 text-xs bg-black/60">فيديو</Badge>
          </div>
        ) : (
      ...
      <div className="p-2">
        <div className="text-xs font-semibold truncate" title={asset.title}>{asset.title}</div>
        ...
        <div className="flex gap-1 mt-1.5">
          ...
          <Button size="sm" variant="outline" className="h-7 px-2 text-red-600 hover:text-red-700" onClick={onDelete}><Trash2 className="w-3 h-3" /></Button>
        </div>
      </div>
    </div>
  );
...
function UploadDialog({ open, onClose, category, allowVideo, acceptDesign, showPlatform, onUploaded }: any) {
  const { toast } = useToast();
  ...
  const accept = useMemo(() => {
    ...
    if (acceptDesign) types.push(".psd,.ai,.indd,.pdf,.fig,.sketch,.eps");
    return types.join(",");
  }, [allowVideo, acceptDesign]);
  ...
  return (
    <Dialog open={open} onOpenChange={() => { if (!uploading) { reset(); onClose(); } }}>
      <DialogContent dir="rtl" className="max-w-2xl">
        ...
        <div className="space-y-3">
          <div
            className="border-2 border-dashed border-amber-300 rounded-lg p-6 text-center cursor-pointer hover:bg-amber-50/50"
  ...
  return (
    <Dialog open={!!asset} onOpenChange={onClose}>
      <DialogContent dir="rtl" className="max-w-4xl">
        ...
        <div className="bg-slate-900 rounded flex items-center justify-center min-h-[300px] max-h-[70vh] overflow-auto">
        ...
        <DialogFooter>
          <a href={`/api/media/assets/${asset.id}/download`}><Button><Download className="w-4 h-4 ml-1" /> تنزيل</Button></a>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
  ...
  return (
    <Dialog open={!!asset} onOpenChange={onClose}>
      <DialogContent dir="rtl">
        ...
        <DialogFooter>
          <Button variant="outline" onClick={onClose}>إلغاء</Button>
          <Button onClick={() => m.mutate()} disabled={m.isPending}>{m.isPending && <Loader2 className="w-4 h-4 animate-spin ml-2" />} حفظ</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
