...
import { buildProductionReportTables, compactPreparationEvidenceReferences, downloadProductionReportCsv, downloadProductionReportExcel, formatQuantity6, getPreparationSourceReadout, preparationSourceQuantityText, printProductionReport, type ProductionReportFilteredRows } from "@/lib/production-operations-export";
...
function SourceQuantity({ row, field }: { row: ProductionOperationsRequestRow; field: "preparedFromStock" | "preparedFromProduction" }) {
  const source = getPreparationSourceReadout(row);
  const value = (row as unknown as Record<string, unknown>)[field];
  return <span className="text-xs tabular-nums">{source.status === "recorded" ? `${preparationSourceQuantityText(value, source)} ${row.unit}` : preparationSourceQuantityText(value, source)}</span>;
}
function SourceEvidence({ row }: { row: ProductionOperationsRequestRow }) {
  const evidence = compactPreparationEvidenceReferences((row as unknown as Record<string, unknown>).productionFulfillmentEvidence);
  return <span className="text-xs text-muted-foreground">{evidence}</span>;
}
...
function ReportBody({ report, rows, search }: { report: ProductionOperationsReport; rows: ProductionReportFilteredRows; search: string }) {
  ...
  return <div className="space-y-5"><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Metric icon={Factory} label="دفعات مكتملة" value={report.summary.production.finishedBatchCount} /><Metric icon={Truck} label="طلبات المطبخ" value={report.summary.centralKitchen.orderCount} /><Metric icon={Warehouse} label="حركات المواد" value={report.summary.materials.movementCount} /><Metric icon={ClipboardList} label="صفوف معروضة" value={allCount} note={search ? "بعد البحث؛ الملخصات أدناه للفترة كاملة." : "في الفترة المحددة"} /></div>
    ...
    <Card><SectionHead icon={ClipboardList} title="الخطة المسجلة" description="مقارنة الدفعات المرتبطة صراحة فقط عبر كامل فترة الخطة؛ التاريخي غير المرتبط غير معلوم. الملغى لا يغطي الخطة." count={rows.planned.length} /><CardContent>{rows.planned.length ? <DataTable caption="الخطة المسجلة"><thead className="bg-muted/50"><tr><th>المنتج</th><th>وحدة التنفيذ / الكتالوج</th><th>المخطط</th><th>قيد التنفيذ</th><th>المكتمل</th><th>متبقٍ للإنشاء</th><th>مراجع الأوامر</th><th>المقارنة</th></tr></thead><tbody>{rows.planned.map((row) => <tr className="border-t" key={`${row.productId}-${row.advancedOrderItemIds.join("-")}`}><td className="font-medium">{row.productName}</td><td>{row.executionUnit || row.catalogUnit || "غير محددة"}</td><td>{formatQuantity6(row.plannedQuantity)}</td><td>{row.linkedInProgressQuantity == null ? "غير معلوم" : formatQuantity6(row.linkedInProgressQuantity)}</td><td>{row.linkedFinishedQuantity == null ? "غير معلوم" : formatQuantity6(row.linkedFinishedQuantity)}</td><td>{row.remainingQuantity == null ? "غير معلوم" : formatQuantity6(row.remainingQuantity)}</td><td className="text-xs">{references(row.advancedOrderIds, "أمر")}</td><td className="text-muted-foreground">{row.comparisonStatus === "available_explicit_batch_order_item_fk" ? "رابط بند صريح" : "غير متاحة دون رابط دفعة مؤكد"}</td></tr>)}</tbody></DataTable> : <EmptyRows searched={Boolean(search)} label="خطط" />}</CardContent></Card>
    <div className="space-y-4"><div><h2 className="font-semibold">مسار طلبات المطبخ</h2><p className="text-xs text-muted-foreground">فصل صريح بين الفعلي والظلّي وغير المعروف. مصدر التجهيز يصف الكمية المجهزة فقط، وليس إنتاجاً إضافياً أو كمية مرسلة.</p></div>{(["real", "shadow", "unknown"] as PipelineMode[]).map((mode) => <ModeSection key={mode} report={report} rows={rows.requests.filter((row) => row.inventoryMode === mode)} mode={mode} searched={Boolean(search)} />)}</div>
