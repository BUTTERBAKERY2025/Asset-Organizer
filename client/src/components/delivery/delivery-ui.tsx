...
const stages: Record<Delivery["status"], { label: string; className: string }> = {
  assigned: { label: "بانتظار الانطلاق", className: "border-violet-200 bg-violet-50 text-violet-800" },
...
export function DeliveryDetail({ delivery, proof }: { delivery: Delivery; proof?: { signatureData: string | null; receiverName: string | null; proofAt: string | null; receiptApprovedBy: string | null; receiptApprovedAt: string | null } }) {
  const receiptPath = delivery.sourceType === "kitchen" ? `/central-kitchen-orders?orderId=${delivery.sourceId}` : `/transfer-requests?transferId=${delivery.sourceId}`;
  return <div className="space-y-4" dir="rtl">
    ...
    {delivery.status === "awaiting_receipt" && <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-950"><span className="flex items-center gap-2"><CircleAlert className="h-5 w-5" />التوقيع قُدم، بانتظار اعتماد الاستلام في المصدر.</span><Link href={receiptPath} className="font-bold underline underline-offset-4">فتح شاشة الاستلام</Link></div>}
