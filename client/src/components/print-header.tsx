...
export const SYSTEM_NAME = "BUTTER BAKERY SYSTEM";
export const SYSTEM_SUBTITLE = "CEO COMMAND";
...
export function PrintHeader({ title, subtitle, showDate = true }: PrintHeaderProps) {
  ...
  return (
    <div className="print-header hidden print:block mb-8 text-center border-b-2 border-[#d4a853] pb-6">
      ...
      />
      <div className="mb-3">
        <p className="text-xs font-bold text-gray-600 tracking-wider">{SYSTEM_NAME}</p>
        <p className="text-[10px] text-[#e67e22] font-medium tracking-wider">{SYSTEM_SUBTITLE}</p>
      </div>
...
export function PrintFooter() {
  ...
  return (
    <div className="print-footer hidden print:block mt-8 pt-4 border-t border-gray-300 text-center text-xs text-gray-500">
      <p>تم إنشاء هذا التقرير بواسطة {SYSTEM_NAME} - {SYSTEM_SUBTITLE}</p>
      <p>{currentDateTime}</p>
    </div>
  );
}
