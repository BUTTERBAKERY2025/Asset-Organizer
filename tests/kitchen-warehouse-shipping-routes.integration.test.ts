...
let manager: any, receiver: any, outsider: any;
...
describe("kitchen warehouse shipping against local PostgreSQL", () => {
  beforeAll(async () => {
    ...
    try {
      ...
      for (const user of [manager,receiver,outsider])
        await q("INSERT INTO users(id,first_name,role,branch_id,is_active) VALUES($1,$1,$2,$3,'active')",[user.id,user.role,branch]);
